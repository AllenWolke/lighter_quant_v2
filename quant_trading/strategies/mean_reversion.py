...
class MeanReversionStrategy(BaseStrategy):
    ...
    def __init__(self, config: Config, market_id: int = 0, 
        ...
        self.market_risk_config = mr_config.get('market_risk_config', {})
        ...
        config_slippage = mr_config.get('market_slippage_config', {})
        if config_slippage:
            # 使用config.yaml中的配置
            ...
            self.market_slippage_config = {
                0: {"enabled": True, "tolerance": 0.008},   # ETH: 0.8%滑点容忍度，均值回归对滑点敏感
                1: {"enabled": True, "tolerance": 0.005},   # BTC: 0.5%滑点容忍度
                2: {"enabled": True, "tolerance": 0.015},   # SOL: 1.5%滑点容忍度
                3: {"enabled": True, "tolerance": 0.025},   # DOGE: 2.5%滑点容忍度，波动较大
            }
        ...
        # ⭐ 需求①：K线类型配置
        self.kline_types = mr_config.get('kline_types', [1])  # 默认只对1分钟K线发出信号
