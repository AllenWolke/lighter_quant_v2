...
check_config() {
    ...
    if [ ! -f "config_linux_mainnet.yaml" ]; then
        log_error "未找到主网配置文件 config_linux_mainnet.yaml"
        log_info "请先复制并配置主网配置文件"
        exit 1
    fi
...
import yaml
try:
    with open('config_linux_mainnet.yaml', 'r', encoding='utf-8') as f:
...
check_network() {
    ...
    # 检查HTTPS连接（比ping更可靠）
    log_info "测试HTTPS连接..."
    if curl -I https://mainnet.zklighter.elliot.ai --connect-timeout 10 > /dev/null 2>&1; then
        log_success "Lighter主网HTTPS连接正常"
    else
        log_warning "HTTPS连接异常，请检查网络设置"
    fi
    
    # 注意：不使用ping，因为很多服务器禁用ICMP响应
}
...
check_resources() {
    ...
    if [ $total_mem -lt 8192 ]; then
        log_warning "内存不足 ($total_mem MB)，推荐16GB+"
    else
        log_success "内存充足 ($total_mem MB)"
    fi
    
    # 检查磁盘空间
    disk_usage=$(df /home/trader 2>/dev/null | tail -1 | awk '{print $5}' | sed 's/%//')
...
async def test_connection():
    try:
        config = Config.from_file('config_linux_mainnet.yaml')
...
" && log_success "Lighter连接测试通过" || {
        log_error "Lighter连接测试失败"
        log_warning "请检查网络连接和配置文件"
    }
}
...
start_trading() {
    ...
    # 检查配置文件中的私钥
    if grep -q "YOUR_MAINNET_PRIVATE_KEY_HERE" config_linux_mainnet.yaml; then
        log_error "请先配置主网私钥"
        log_info "编辑 config_linux_mainnet.yaml 文件"
        return 1
    fi
    
    log_warning "即将启动主网交易系统，请确认："
    log_warning "1. 已配置正确的主网私钥"
    log_warning "2. 已设置适当的风险参数"
    log_warning "3. 已备份重要数据"
    echo
    ...
    # 启动交易系统
    nohup python3 main.py --config config_linux_mainnet.yaml > logs/trading.log 2>&1 &
    trading_pid=$!
...
start_service() {
    ...
    if systemctl is-active --quiet lighter-trading; then
        log_warning "服务已在运行"
        systemctl status lighter-trading
    else
...
run_backtest() {
    ...
    case $backtest_choice in
        1)
            log_info "运行动量策略回测..."
            python3 run_backtest.py --strategy momentum --start-date 2024-01-01 --end-date 2024-01-31 --config config_linux_mainnet.yaml
            ;;
        2)
            log_info "运行均值回归策略回测..."
            python3 run_backtest.py --strategy mean_reversion --start-date 2024-01-01 --end-date 2024-01-31 --config config_linux_mainnet.yaml
            ;;
        3)
            ...
            python3 run_backtest.py --strategy $strategy_name --start-date $start_date --end-date $end_date --config config_linux_mainnet.yaml
            ;;
        *)
            log_error "无效选择"
            ;;
...
system_monitor() {
    ...
    if [ -f "monitor_mainnet.py" ]; then
        ...
        echo
        echo "网络状态:"
        if ping -c 1 mainnet.zklighter.elliot.ai > /dev/null 2>&1; then
            echo "  Lighter连接: 正常"
        else
            echo "  Lighter连接: 异常"
        fi
